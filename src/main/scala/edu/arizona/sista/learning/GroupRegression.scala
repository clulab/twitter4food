...
import edu.stanford.nlp.kbp.slotfilling.classify.{JointlyTrainedRelationExtractor, ModelType, MultiLabelDataset}
import edu.stanford.nlp.kbp.slotfilling.common.{Constants, Log, Props}
import edu.stanford.nlp.ling.{RVFDatum => S_RVFDatum}
import edu.stanford.nlp.ling.{Datum => S_Datum}
import edu.stanford.nlp.ling.{BasicDatum => S_BasicDatum}
import edu.stanford.nlp.stats.{ClassicCounter, Counter, Counters}
import edu.stanford.nlp.util.{ErasureUtils, HashIndex, Index}
;


trait LocalClassificationMode

case object WeightedVote extends LocalClassificationMode
case object SingleModel extends LocalClassificationMode

/*
trait LocalFilter {
  def filterZ(data: Array[Datum[String, String]]): Boolean

  def filterY(data: Array[Datum[String, String]]): Boolean = {
    return true
  }
}

case class LargeFilter(threshold: Int = 0) extends LocalFilter {

  def filterZ(data: Array[Datum[String, String]]): Boolean = data.length <= threshold

  override def filterY(data: Array[Datum[String, String]]): Boolean = data.length <= threshold
}

case object AllFilter extends LocalFilter {
  def filterZ(data: Array[Datum[String, String]]): Boolean = true
}
*/
...
object GroupRegression {
  ...
  protected val localClassificationMode: LocalClassificationMode = WeightedVote
  ...
  private def makeLocalData(dataArray: Array[Array[S_Datum[String, String]]], labelIndex: Index[String], featureIndex: Index[String], fold: Int, useRVF: Boolean, initialZLabels: Array[Array[String]]): GeneralDataset[String, String] = {
    ...
    for ((group, zLabels) <- dataArray zip initialZLabels) {
      for ((datum, label) <- (group zip zLabels)) {
        if (useRVF) {
          val d = datum.asInstanceOf[S_RVFDatum[String, String]]
          d.setLabel(label)
          (dataset.asInstanceOf[WeightedRVFDataset[String, String]]).add(d, 1.0f)
        } else {
          (datum.asInstanceOf[S_BasicDatum[String, String]]).setLabel(label)
...
class GroupRegression(val positiveClass: String,
  ...
  def train(data: MultiLabelDataset[String, String], initialZLabels: Array[Array[String]]) {
    ...
    {
      var epoch: Int = 0
      while (epoch < numberOfTrainEpochs) {
        {
          zUpdatesInOneEpoch = 0
          Log.severe("***EPOCH " + epoch + "***")
          val zLabelsPredictedByZ: Array[Array[Int]] = new Array[Array[Int]](zLabels.length)
          {
            var i: Int = 0
            while (i < zLabels.length) {
              zLabelsPredictedByZ(i) = new Array[Int](zLabels(i).length)
              ({
                i += 1; i - 1
              })
            }
            ...
            while (fold < numberOfFolds) {
              {
                ...
                val end: Int = foldEnd(fold, data.getDataArray.length)
                {
                  ...
                  while (i < end) {
                    {
                      val group: Array[S_Datum[String, String]] = data.getDataArray(i)
            ...
            while (fold < numberOfFolds) {
              {
                ...
                val foldTrainArray: Array[Array[Int]] = makeTrainDataArrayForFold(zDataset.getDataArray, fold)
                val foldTrainLabels: Array[Int] = makeTrainLabelArrayForFold(zDataset.getLabelsArray, fold)
          ...
          {
            var i: Int = 0
            while (i < yDataset.getLabelsArray.length) {
              {
  ...
  private[twitter4food] def randomizeGroup(group: Array[S_Datum[String, String]], randomSeed: Int) {
    val rand: Random = new Random(randomSeed)
    {
      var j: Int = group.length - 1
      while (j > 0) {
        {
          val randIndex: Int = rand.nextInt(j)
          val tmp: S_Datum[String, String] = group(randIndex)
  ...
  protected def makeTrainLabelArrayForFold(labelArray: Array[Int], fold: Int): Array[Int] = {
    val start: Int = foldStart(fold, labelArray.length)
    val end: Int = foldEnd(fold, labelArray.length)
    val train: Array[Int] = new Array[Int](labelArray.length - end + start)
  ...
  protected def makeTrainDataArrayForFold(dataArray: Array[Array[Int]], fold: Int): Array[Array[Int]] = {
    val start: Int = foldStart(fold, dataArray.length)
    val end: Int = foldEnd(fold, dataArray.length)
    val train: Array[Array[Int]] = new Array[Array[Int]](dataArray.length - end + start)
    var trainOffset: Int = 0
    {
      var i: Int = 0
      while (i < start) {
  ...
  @SuppressWarnings(Array("unchecked")) protected def initializeZClassifierLocally(data: MultiLabelDataset[String, String], featureIndex: Index[String], labelIndex: Index[String], initialZLabels: Array[Array[String]]): Array[LinearClassifier[String, String]] = {
    val localClassifiers: Array[LinearClassifier[String, String]] = new Array[LinearClassifier[_, _]](numberOfFolds)
    {
      var fold: Int = 0
      while (fold < numberOfFolds) {
        {
            ...
            while (i < testDataArray.length) {
              {
                ...
                val pred: java.util.Set[Integer] = new java.util.HashSet[Integer]
                {
                  for (j <- 0 until group.length) {
                    {
                      val datum: S_Datum[String, String] = group(j)
  ...
  protected def initializeZDataset(totalSentences: Int, zLabels: Array[Array[Int]], data: Array[Array[S_Datum[String, String]]]): GeneralDataset[String, String] = {
    ...
    var count: Int = 0
    {
      ...
      while (i < data.length) {
        {
          val group: Array[S_Datum[String, String]] = data(i)
          {
            var s: Int = 0
            while (s < group.length) {
              {
                val datum: S_Datum[String, String] = group(s)
                if (useRVF) {
                  (datum.asInstanceOf[S_RVFDatum[String, String]]).setLabel(zLabelIndex.get(zLabels(i)(s)))
                }
                else {
                  (datum.asInstanceOf[S_BasicDatum[String, String]]).setLabel(zLabelIndex.get(zLabels(i)(s)))
                }
                dataset.add(datum)
                count += 1
              }
  ...
  private[twitter4food] def predictZLabels(group: Array[S_Datum[String, String]], zLabels: Array[Int], zClassifier: LinearClassifier[String, String]) {
    {
      var s: Int = 0
      while (s < group.length) {
        {
          val probs: Counter[String] = zClassifier.logProbabilityOf(group(s))
  ...
  private[twitter4food] def computeZLogProbs(group: Array[S_Datum[String, String]], zLogProbs: Array[Counter[String]], zClassifier: LinearClassifier[String, String], epoch: Int) {
    {
      var s: Int = 0
      while (s < group.length) {
        {
  ...
  private[twitter4food] def inferZLabelsStable(group: Array[S_Datum[String, String]], positiveLabels: Set[Integer], negativeLabels: Set[Integer], zLabels: Array[Int], zLogProbs: Array[Counter[String]], zClassifier: LinearClassifier[String, String], epoch: Int) {
    ...
    computeZLogProbs(group, zLogProbs, zClassifier, epoch)
    {
      ...
      while (s < group.length) {
        {
          var maxProb: Double = Double.NEGATIVE_INFINITY
          var bestLabel: Int = -1
          ...
          for (candidate <- zProbabilities.keySet) {
            ...
            val yLabel: String = yLabelIndex.get(positiveLabels.iterator.next)
            val yDatum: S_Datum[String, String] = new S_RVFDatum[String, String](extractYFeatures(zLabels), "")
   ...
   */
  def classifyLocally(datum: S_Datum[String, String]): Counter[String] = {
    if (GroupRegression.localClassificationMode eq GroupRegression.LOCAL_CLASSIFICATION_MODE.WEIGHTED_VOTE) {
      val sumProbs: Counter[String] = new ClassicCounter[String]
      {
        ...
        while (fold < numberOfFolds) {
          {
            val zClassifier: LinearClassifier[String, String] = zClassifiers(fold)
            val probs: Counter[String] = zClassifier.probabilityOf(datum)
    ...
    if (GroupRegression.localClassificationMode eq GroupRegression.LOCAL_CLASSIFICATION_MODE.SINGLE_MODEL) {
      val probs: Counter[String] = zSingleClassifier.probabilityOf(datum)
      return probs
    }
    throw new RuntimeException("ERROR: classification mode " + GroupRegression.localClassificationMode + " not supported!")
  }

  def classifyMentions(sentences: List[S_Datum[String, String]]): Counter[String] = {
