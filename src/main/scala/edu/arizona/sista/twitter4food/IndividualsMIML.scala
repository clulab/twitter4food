...
object IndividualsMIML {
  ...
  def makeTrainingAndTesting(trainingCorpus: IndividualsCorpus, testingCorpus: Option[LabelledIndividualsCorpus]): (Seq[IndividualsTweets], Seq[IndividualsTweets]) = {
    ...
    def makeIndividuals(tweetsByUserByState: Map[String, Map[String, Seq[Tweet]]]): Seq[IndividualsTweets] = for {
      (state, tweetsByUser) <- tweetsByUserByState.toSeq
      (username, tweets) <- tweetsByUser
    } yield IndividualsTweets(tweets, username, None, Some(state))

    val (trainingTweets, testingTweets) = testingCorpus match {
      case None => {
        (makeIndividuals(trainingCorpus.trainingTweets), makeIndividuals(trainingCorpus.testingTweets))
      }
      case Some(testC) => {
        (makeIndividuals(trainingCorpus.allTweets), testC.tweets)
      }
    }
  ...
  def main(args: Array[String]) {
    ...
    val (trainingTweets, testingTweets) = makeTrainingAndTesting(individualsCorpus, testCorpus)
