...
import edu.arizona.sista.learning.{LiblinearClassifier, Classifier}
import edu.arizona.sista.struct.Counter
import edu.arizona.sista.utils.{EvaluationStatistics, StringUtils}

import scala.collection.mutable

class StatesToIndividuals(parameters: ExperimentParameters, printWriter: PrintWriter = new java.io.PrintWriter(System.out))
...
object StatesToIndividuals {
  ...
  def main(args: Array[String]) {
    ...
    val removeMarginals: Option[Int] = Some(20)
    ...
    val individualsCorpus = new IndividualsCorpus("/data/nlp/corpora/twitter4food/foodSamples-20150501", "/data/nlp/corpora/twitter4food/foodSamples-20150501/annotations.csv", annotatedTestingFraction = 1.0, numToTake=Some(500))
    ...
    //tokenTypes: TokenType <- List(AllTokens, HashtagTokens, FoodTokens, FoodHashtagTokens).par
      ...
      maxTreeDepth: Option[Int] <- List(Some(6),None)
      numTrees <- List(6,9,1000)
